...
onMounted(() => {
  ...
  cameraController.camera.position.set(12.12098385770267, 13.467045544205337, 26.738723988249237);
  cameraController.controls.target.set(3.3909695398082227, 7.951344708497408, 6.530731078615542);

  cameraController.camera.fov = 25;
  cameraController.controls.update();

  const catFallScene = ScenePreloadService.getAsset('owlNeckScene').clone();
  render3d.scene.add(catFallScene);

  const model = ScenePreloadService.getAsset('owlNeck');
  model.name = 'Group';
  ...
  render3d.scene.traverse((child) => {
    ...
    if (child.isMesh) {
      ...
      if (child.material.name === 'sel_MAT.001') {
